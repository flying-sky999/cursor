...
?????? Playwright ? https://pika.art/pikaffects ???????
???????????????????
"""
...
class PikaFFectsTest:
    """PikaFFects ??????"""
    
    def __init__(self, headless: bool = False):
        self.headless = headless
        self.base_url = "https://pika.art/pikaffects"
        self.test_results = []
    ...
    async def test_page_load(self) -> bool:
        ...
        try:
            ...
            # ????
            await self.page.screenshot(path="screenshots/page_loaded.png")
            self.log("???????", test_name)
            ...
            await self.page.screenshot(path="screenshots/page_load_error.png")
            return False
    ...
    async def test_page_title(self) -> bool:
        ...
        try:
            ...
            # ?????? Pika ?????
            if "pika" in title.lower() or "pikaffects" in title.lower():
    ...
    async def test_responsive_design(self) -> bool:
        ...
        try:
            ...
            for device_name, width, height in viewports:
                ...
                await self.page.screenshot(path=f"screenshots/{device_name}_{width}x{height}.png")
...
async def main():
    ...
    # ??????
    tester = PikaFFectsTest(headless=headless)
